...
- **yb-\*-yieldbearing apr = official 7d trading APY** — switched 2026-08-25 from all-time / inception PPS growth (`tradingApyAllTime`) to Yield Basis's official 7-day fundamental trading APY (`tradingApy1w` via `/v1/analytics/markets/trading-apy`). That matches the trailing window most BitcoinYield adapters publish. TVL remains on-chain from `updated_balances()` / `pricePerShare()`. The floor-at-0 + `metadata.rawApr7d` + conditional `allowZeroApr` pattern is retained, so a negative 7d window stores 0 without hiding a frozen feed.
