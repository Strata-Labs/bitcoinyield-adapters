...
test("a floored zero with allowZeroApr passes normalize and boundaries", async () => {
    ...
    [
      {
        ...
        tvlBtc: 129.8,
        apr: Math.max(-0.37, 0),
        metadata: { allowZeroApr: true, rawApr7d: -0.37 },
      },
    ],
    adapter,
    100_000,
  ...
  assert.equal(rows[0]?.metadata?.rawApr7d, -0.37);
...
test("yieldbasis yield-bearing adapters floor a negative official 7d APY", async () => {
