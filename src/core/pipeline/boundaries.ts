...
function check(
  ...
  | { field: 'tvlBtc' | 'apr'; value: number; bound: 'lower' | 'upper'; threshold: number }
  | null {
  if (row.tvlBtc < BOUNDARIES.tvlBtc.lb) {
    return { field: 'tvlBtc', value: row.tvlBtc, bound: 'lower', threshold: BOUNDARIES.tvlBtc.lb }
  }
  if (row.tvlBtc > BOUNDARIES.tvlBtc.ub) {
    return { field: 'tvlBtc', value: row.tvlBtc, bound: 'upper', threshold: BOUNDARIES.tvlBtc.ub }
  }
  if (row.apr < BOUNDARIES.apr.lb) {
    return { field: 'apr', value: row.apr, bound: 'lower', threshold: BOUNDARIES.apr.lb }
  }
  if (row.apr > BOUNDARIES.apr.ub) {
    return { field: 'apr', value: row.apr, bound: 'upper', threshold: BOUNDARIES.apr.ub }
  }
  return null
}
