...
- **yb-\*-yieldbearing apr = all-time PPS growth** — switched 2026-08-19 from trailing-30d PPS growth to all-time annualized PPS growth since market creation, matching the "FT APY" on the yieldbasis.com dashboard (their `tradingApyAllTime`; genesis PPS is exactly 1.0, so no archive read is needed). The trailing-30d figure had gone ~0 during the "PPS drag" period (rebalancing costs outran trading fees) and read as a broken adapter. The floor-at-0 + `metadata.rawAprAllTime` + conditional `allowZeroApr` pattern is retained for the theoretical pps<1 case, so a frozen feed still fails loudly.
