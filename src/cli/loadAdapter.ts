...
export function listAdapterSlugs(): string[] {
  ...
  return readdirSync(ADAPTERS_DIR, { withFileTypes: true })
    .filter((d) => d.isDirectory() && existsSync(path.join(ADAPTERS_DIR, d.name, 'index.ts')))
    .map((d) => d.name)
    .sort()
}
...
export async function loadAdapter(slug: string): Promise<Adapter> {
  ...
  const mod = (await import(pathToFileURL(indexPath).href)) as { default: Adapter }
  if (!mod.default) {
    throw new Error(`Adapter ${slug} must default-export the result of defineAdapter(...)`)
  }
  return mod.default
}
