...
export class HttpStorage implements Storage {
  ...
  constructor(options: HttpStorageOptions) {
    ...
    if (!options.adapterKey) throw new Error('HttpStorage: adapterKey is required')
  ...
  async insert(slug: string, rows: MetricRow[]): Promise<void> {
    ...
    const [row] = rows
    const res = await this.request('POST', '/api/adapter-metrics', { slug, row })
    if (!res.ok) {
      throw new Error(
  ...
  async recordRun(slug: string, record: AdapterRunRecord): Promise<void> {
    const res = await this.request('POST', '/api/adapter-status', { slug, ...record })
    if (!res.ok) {
      throw new Error(
